...
export class ParticleFluid {
    constructor(renderer, scene, uiSettings) {
        ...
        this.sphUniforms = {
            ...
            u_viscosityMultiplier: { value: 0.55 },  // higher for gentle laminar flow
            u_mass: { value: 1.0 },
            ...
            u_agitation: { value: 0.02 },            // near-zero turbulence
    ...
    getVelocityShader() {
        return `
            ...
            void main() {
                ...
                vec3 pos1 = posData.xyz;
                vec3 vel1 = texture2D(textureVelocity, uv).xyz;

                // Toroidal Z wrap: nudge with deterministic jitter + mild downstream bias
                // so the whole population does not stack on one slab after respawn.
                if (posData.w < 0.5) {
                ...
                // Accent rock interactions (subtle creek-wake repulsion)
                ...
                // Hard bounce only for Y-floor (terrain is separate below) and Z in non-periodic mode.
                // X walls and Y-ceiling use soft repulsion so particles can't pile against them.
                ...
                // Soft lateral (±X) boundary: repulsion ramp widened to 10% of half-width
                // and strength raised to reduce persistent particle adhesion on the banks.
                const float xSoftZone = 0.10;
                if (localPos.x > (0.5 - xSoftZone)) {
                    float pen = (localPos.x - (0.5 - xSoftZone)) / xSoftZone;
                    localVel.x -= pen * pen * 7.5;
                }
                if (localPos.x < -(0.5 - xSoftZone)) {
                    float pen = (-(0.5 - xSoftZone) - localPos.x) / xSoftZone;
                    localVel.x += pen * pen * 7.5;
                }

                // Soft Y-ceiling: prevents airborne splashes from hard-bouncing off the roof.
                ...
                // Conditional wall damping: reduce high-speed events near any wall.
                ...
                gl_FragColor = vec4(clamp(vel1, vec3(-16.0), vec3(16.0)), 1.0);
            }
        `;
    }
    ...
    setRocks(rockList) {
        ...
        this.velocityVariable.material.uniforms.u_rockPositions.value =
            this.sphUniforms.u_rockPositions.value;
        this.velocityVariable.material.uniforms.u_rockRadii.value =
